...
"SELECT projects.title, (SUM(pledges.amount - projects.funding_goal)) AS project_total FROM projects
JOIN pledges ON projects.id = pledges.project_id
GROUP BY projects.title
HAVING project_total >= 0"
end

def selects_user_names_and_amounts_of_all_pledges_grouped_by_name_then_orders_them_by_the_amount_and_users_name
"SELECT users.name, SUM(pledges.amount) AS all_pledges FROM users
INNER JOIN pledges ON users.id = pledges.user_id
GROUP BY users.name ORDER BY all_pledges"
end
